// תור אחד להדגמת "הרכב מוכן", עם טלפון אמיתי של מי שמדגים.
//
// הרצה (מתוך levi-garage):
//   node --env-file=.env.local scripts/seed-demo-notice.mjs
//
// הטלפון נלקח מ-DEMO_PHONE שבקובץ המקומי
// 03-rollout/solution-3-agent/.env.wiring.local, ולא מודפס. שם אופציונלי:
// DEMO_NAME באותו קובץ.
//
// זה קיצור דרך להדגמה מוקלטת, במקום לקבוע תור ב-Cal.com. הבוט עדיין שולח
// רק למי שכתב למוסך ב-24 השעות האחרונות, ולכן לפני ההדגמה צריך לשלוח
// מהטלפון הזה הודעה כלשהי לוואטסאפ של המוסך.
//
...
const first = (local.get("DEMO_PHONE") || "").replace(/\D/g, "")
if (!/^972\d{8,9}$/.test(first)) {
  console.error(`
✗ אין מספר תקין ב-DEMO_PHONE בקובץ:
  ${wiringFile}
  ספרות עם קידומת מדינה, למשל: DEMO_PHONE=9725XXXXXXXX
`)
  process.exit(1)
}
...
console.log(`✓ התור מוכן להדגמה (הטלפון לא מודפס).
  0. מהטלפון: לשלוח הודעה כלשהי לוואטסאפ של המוסך (אחרת הבוט לא ישלח)
  1. להתחבר כדניאל ולפתוח את "לוח היום"
  2. לפתוח כרטיס לסקודה FABIA
  3. בכרטיס: "הרכב מוכן" — ההודעה יוצאת לטלפון`)
